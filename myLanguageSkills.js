...
function myLanguages(results) {
  ...
  const filteredArray = arr.filter(([lang, score]) => score >= 60);
  const sortedFilteredArray = filteredArray.sort((a, b) => b[1] - a[1]);
  const sortedLanguages = sortedFilteredArray.map((x) => x[0]);
